...
<form method="POST" action="{{ $action }}" enctype="multipart/form-data" class="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    ...
    @if($news->image_path)
        @php
            $newsImg = str_starts_with($news->image_path, 'http') ? $news->image_path : (str_starts_with($news->image_path, 'images/') ? asset($news->image_path) : asset('storage/' . $news->image_path));
        @endphp
        <div>
            <p class="text-sm font-bold text-slate-700">Current image</p>
            <img src="{{ $newsImg }}" onerror="this.onerror=null; this.src='{{ asset('images/institute-gallery-1.png') }}';" alt="Current image for {{ $news->title }}" class="mt-2 h-32 w-48 rounded-xl object-cover">
        </div>
    @endif
