...
<form method="POST" action="{{ $action }}"
    ...
    <div class="grid gap-8 md:grid-cols-3">
        <div class="md:col-span-2">
            <label class="{{ $labelClass }}">Semester Name *</label>
            <input name="name" value="{{ old('name', $semester?->name) }}" required maxlength="255"
                placeholder="e.g. 1st Semester / First Semester" class="{{ $inputClass }}">
        ...
        <div>
            ...
            <input type="number" name="sort_order" min="0" value="{{ old('sort_order', $semester?->sort_order ?? 0) }}"
                class="{{ $inputClass }}">
        </div>
    </div>
    ...
    <div class="flex flex-col md:flex-row items-center justify-between gap-6 border-t border-white/5 pt-8">
        <label class="group relative flex cursor-pointer items-center gap-4 rounded-2xl border border-white/5 bg-[#071c2c]/50 px-8 py-5 transition hover:bg-[#071c2c]">
            <div class="flex h-6 w-11 shrink-0 items-center rounded-full bg-slate-700 p-1 transition duration-300 peer-checked:bg-emerald-600">
                ...
                <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $semester?->is_active ?? true))
                    class="peer sr-only">
            ...
            <div>
                ...
                <p class="text-[9px] font-bold text-slate-500 uppercase tracking-tight">Enable this semester for result entry</p>
            </div>
        </label>
