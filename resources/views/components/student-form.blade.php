...
@php
    $inputClass =
        'w-full rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-800 outline-none transition focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500';
...
<div class="space-y-6" data-student-registration-container>
    ...
    <div class="rounded-2xl bg-[#03224c] p-1.5 shadow-lg">
        <div class="grid grid-cols-1 gap-1.5 sm:grid-cols-3">
            @foreach ([['passport', 'Passport Scan', 'Scan Passport for Auto-Fill'], ['nid', 'NID Scan', 'Scan National ID (NID) for Auto-Fill'], ['birth', 'Birth Registration Scan', 'Scan Birth Registration (জন্ম নিবন্ধন) for Auto-Fill']] as [$key, $label, $desc])
                <button type="button"
                    class="group relative flex items-center gap-3 rounded-xl border border-white/5 bg-[#0e315e] px-4 py-3 text-left transition hover:bg-[#15417a] {{ $key === 'birth' ? 'ring-2 ring-amber-500/50' : '' }}"
                    data-scan-tab="{{ $key }}">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="truncate text-[9px] font-bold text-[#6cb2eb]/70 tracking-tight">{{ $desc }} for Auto-Fill</p>
                    </div>
                </button>
            @endforeach
        </div>
    ...
    <form method="POST" action="{{ $action }}" enctype="multipart/form-data" data-location-form
        ...
        <div class="flex flex-col justify-center gap-4 pt-4 sm:flex-row">
            ...
            @if ($cancelRoute)
                <a href="{{ $cancelRoute }}" class="inline-flex min-h-[50px] items-center justify-center rounded-lg border border-slate-400 bg-slate-400/20 px-12 text-[13px] font-black uppercase tracking-widest text-slate-200 transition hover:bg-slate-400/30">
                    Cancel
                </a>
            @endif
        </div>
