<x-dashboard-shell title="Add {{ $section->label }}">
        ...
        <div class="mx-auto max-w-2xl">
            <div class="rounded-[2.5rem] border border-white/20 bg-[#03224c] p-10 shadow-2xl backdrop-blur-sm lg:p-14">
                ...
                <div class="mb-10 text-center">
                    <h1 class="text-4xl font-black text-white uppercase tracking-tight">Student Review</h1>
                    <p class="mt-2 text-[10px] font-black uppercase tracking-[0.2em] text-[#6cb2eb]">Share your experience</p>
                </div>

                <form action="{{ route('super-admin.homepage.items.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    @csrf
                    <input type="hidden" name="section" value="{{ $section->key }}">
                    <input type="hidden" name="stable_key" value="testimonial-{{ time() }}">
                    ...
                    <div class="flex flex-col items-center">
                        <label for="review-photo" class="group relative flex size-32 cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-white/10 bg-[#071c2c]/50 transition hover:border-blue-500/50">
                            <img id="review-photo-preview" src="#" class="absolute inset-0 hidden size-full rounded-2xl object-cover">
                            <div id="review-photo-placeholder" class="flex flex-col items-center gap-2 text-slate-500">
                        ...
                        <p class="mt-4 text-[10px] font-bold uppercase tracking-widest text-slate-400">Upload Profile Photo</p>
                    ...
                    <div>
                        ...
                        <input type="text" name="title" required placeholder="Ex: John Doe"
    ...
    @else
        <div class="mx-auto max-w-4xl py-6">
            <x-homepage-item-form :item="$item" :section="$section" :action="route('super-admin.homepage.items.store')" submit-label="Create item" />
        </div>
    @endif
</x-dashboard-shell>
