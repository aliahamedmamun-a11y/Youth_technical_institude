...
<html lang="en" class="scroll-smooth">
    ...
    <body class="overflow-x-hidden bg-stone-50 text-slate-900 antialiased transition-colors duration-300 dark:bg-ink dark:text-white">
        ...
        <main id="main-content" class="public-page-main">
            ...
            <section id="about" class="overflow-hidden bg-white py-2 dark:bg-deep sm:py-3 lg:py-4" data-about-carousel data-about-interval="10000">
                @if ($aboutEntries->isNotEmpty())
                    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8" data-about-layout="profile" data-about-size="extra-compact">
                        <div class="relative">
                            @foreach ($aboutEntries as $about)
                                @php
                                    $aboutImage = $about->image_path ?: $about->principal_image_path;
                                @endphp
                                <article class="grid items-start gap-4 md:grid-cols-[minmax(200px,0.52fr)_minmax(0,1.65fr)] md:gap-6 lg:grid-cols-[minmax(220px,0.52fr)_minmax(0,1.7fr)] lg:gap-7 {{ $loop->first ? '' : 'hidden' }}" data-about-slide>
                                    <div class="mx-auto w-full max-w-[250px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_8px_20px_rgba(15,23,42,.11)] dark:border-white/10 dark:bg-ink lg:max-w-[270px]">
                                        ...
                                        @if ($about->principal_name)
                                            <div class="bg-[#0b2447] px-3 py-2.5 text-center text-white"><h3 class="text-base font-black tracking-tight sm:text-lg">{{ $about->principal_name }}</h3><p class="text-[11px] text-slate-200">{{ $about->principal_title }}</p></div>
                                        @endif
                                    </div>
                                    <div class="min-w-0 pt-1 md:pt-0">
                                        <div class="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-2 sm:gap-x-5">
                                            <h2 class="min-w-0 max-w-full text-xl font-black tracking-tight text-slate-950 sm:text-2xl dark:text-white" aria-label="About Us" data-about-heading>
                                        ...
                                        <div class="mt-2 h-px w-full bg-slate-200 dark:bg-white/10"></div>
                                        <h3 class="mt-3 text-xs font-semibold text-slate-900 dark:text-white">About the Institution</h3>
                                        @if ($about->summary)
                                            <p class="mt-1 text-[13px] leading-5 text-slate-700 lg:text-xs lg:leading-[18px] dark:text-slate-200">{{ $about->summary }}</p>
                                        @endif
                                        @if ($about->content)
                                            <div class="mt-1 whitespace-pre-line text-[13px] leading-5 text-slate-600 lg:text-xs lg:leading-[18px] dark:text-slate-300">{{ $about->content }}</div>
                                        @endif
                                    </div>
                                </article>
                            @endforeach
